//Program which will determine whether a point lies inside the circle, on the circle or outside the circle.

#include<stdio.h>
#include<math.h>

int main()
{
    int x1,y1,x2,y2,radius;
    float pc;

    printf("Enter the center co-ordinates of the circle: \n");
    scanf("%d %d",&x1,&y1);

    printf("Enter the radius of the circle: \n");
    scanf("%d",&radius);

    printf("Enter the point co-ordinates: \n");
    scanf("%d %d",&x2,&y2);

    pc=sqrt(pow(x2-x1,2)+pow(y2-y1,2));

    if(pc>radius)
        printf("point (%d,%d) lies outside the circle\n",x2,y2);
    else if(pc<radius)
        printf("point (%d,%d) lies inside the circle\n",x2,y2);
    else if(pc==radius)
        printf("point (%d,%d) lies on the boundary of circle",x2,y2);
    else
        printf("wrong entry");
}
